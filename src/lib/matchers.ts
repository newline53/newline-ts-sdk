...
import { NewlineDefaultError } from "../models/errors/newline-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | NewlineDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | NewlineDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new NewlineDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new NewlineDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
